...
def numba_color2gray(image: np.array) -> np.array:
    ...
    # Iterate through the pixels and apply the grayscale transform using Numba
...
def numba_color2sepia(image: np.array) -> np.array:
    ...
    sepia_image = np.empty((height, width, 3), dtype=np.uint8)
    # Iterate through the pixels
    # applying the sepia matrix

    # Iterate through the pixels and apply sepia transformation using Numba JIT
    for i in range(height):
        for j in range(width):
            # Get RGB values of the current pixel
            ...
            sepia_red = min(255, int(red * sepia_matrix[0, 0] + green * sepia_matrix[0, 1] + blue * sepia_matrix[0, 2]))
            sepia_green = min(255, int(red * sepia_matrix[1, 0] + green * sepia_matrix[1, 1] + blue * sepia_matrix[1, 2]))
            sepia_blue = min(255, int(red * sepia_matrix[2, 0] + green * sepia_matrix[2, 1] + blue * sepia_matrix[2, 2]))
